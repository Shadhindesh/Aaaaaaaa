...
class BunnyVideoDRM:
    ...
    def prepare_dl(self) -> str:
        ...
        def main_playlist():
            ...
            resolutions = re.findall(r'\s*(.*?)\s*/video\.drm', response.text)

            # Print available resolutions
            print("Available resolutions:", resolutions)

            # Check for available resolutions
            available_resolutions = {res: int(res[:-1]) for res in resolutions}  # Remove 'p' and convert to int

            for res in ['1080p', '720p']:
                if res in available_resolutions:
                    print(f"{res} is available.")
                else:
                    print(f"{res} is not available.")

            # Return the highest resolution available (if any)
            if not resolutions:
                sys.exit(2)
            else:
                highest_resolution = max(available_resolutions, key=available_resolutions.get)
        ...
        for i in range(0, 29, 4):  # first 28 seconds, arbitrary (check issue#11)
            ping(time=i + round(random(), 6), paused='false', res=resolution.split('p')[0])
...
if __name__ == '__main__':
    data = load_data()
    video = BunnyVideoDRM(
        referer=data['Refer URL'],
        embed_url=data['Embed URL'],
        name=data['Name'],
        path=os.getcwd()
    )
    video.download()